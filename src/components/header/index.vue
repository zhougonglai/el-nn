...
<template>
  <Disclosure as="nav" class="bg-gray-800 drag" v-slot="{ open }">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <router-link v-for="item in navigation" :key="item.name" :to="item.href"
                ...
                active-class="bg-gray-900 text-white" :aria-current="item.current ? 'page' : undefined">{{ item.name }}
              </router-link>
            </div>
          </div>
        </div>
...
const openWin = async () => {
  await window.electronAPI.openWin('/setting')
}
</script>
